...
class MobilePaf():
    ...
    def build(self, features):
        ...
        if self.backbone == 'mobilenet_v1':
            ...
            paf_ch1 = tf.layers.conv2d(pad_paf, 256, kernel_size=[1, 1], name='paf_channel1')  
            ps2 = self.PixelShuffle(paf_ch1, 2, scope='PixelShuffle2')
            paf = tf.layers.conv2d(ps2, 36, kernel_size=[1, 1], name='paf_conv')
            ...
            hm_ch1 = tf.layers.conv2d(pad_hm, 128, kernel_size=[1, 1], name='hm_channel1')
            ps1 = self.PixelShuffle(hm_ch1, 2, scope='PixelShuffle1')
