...
_l2_regularizer_convb = tf.contrib.layers.l2_regularizer(network_common.regularizer_conv)
...
class BaseNetwork(object):
    ...
    def load(self, data_path, session, ignore_missing=False):
        ...
        for op_name, param_dict in data_dict.items():
            if isinstance(data_dict[op_name], np.ndarray):
                ...
                op_name = network_common.to_str(op_name)
                # if op_name > 'conv4':
                ...
                with tf.variable_scope(op_name, reuse=True):
                    for param_name, data in param_dict.items():
                        try:
                            var = tf.get_variable(network_common.to_str(param_name))
    ...
    def separable_conv(self, input, k_h, k_w, c_o, stride, name, relu=True, set_bias=True):
        with slim.arg_scope([slim.batch_norm], decay=0.999, fused=network_common.batchnorm_fused, is_training=self.trainable):
            output = slim.separable_convolution2d(input,
                                                  ...
                                                  kernel_size=[k_h, k_w],
                                                  # activation_fn=network_common.activation_fn if relu else None,
                                                  activation_fn=None,
